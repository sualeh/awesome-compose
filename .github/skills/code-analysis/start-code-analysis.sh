...
PROJECT_PATH="$(grep -E '^[[:space:]]*LOCAL_PROJECT_PATH[[:space:]]*=' "$ENV_FILE" | tail -n 1 | sed -E 's/^[[:space:]]*LOCAL_PROJECT_PATH[[:space:]]*=[[:space:]]*//')"
PROJECT_PATH="${PROJECT_PATH%$'\r'}"

if [[ -z "${PROJECT_PATH// }" ]]; then
  echo "LOCAL_PROJECT_PATH must be set in $ENV_FILE" >&2
  exit 1
fi
