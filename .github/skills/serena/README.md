...
- serena.env.example: Example environment file; copy to `serena.env` and set valid host workspace paths
...
When the user asks to start Serena or to run a local Serena MCP server, ensure `serena.env` exists and points to valid paths. Run the start script from this folder and share the endpoint URL.
