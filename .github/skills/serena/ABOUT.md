# Serena

Serena is a coding-focused assistant framework that can run as an MCP server and expose tools over HTTP. In this repository, it is packaged as a local Docker Compose utility so agents and humans can start and stop it consistently.

This skill runs Serena with streamable HTTP transport on port 9121, mounts one selected local project path, and activates that project automatically at startup.

## References

- [Serena GitHub Repository](https://github.com/oraios/serena)
- [Serena Container Registry](https://github.com/oraios/serena/pkgs/container/serena)
- [Model Context Protocol](https://modelcontextprotocol.io/)
