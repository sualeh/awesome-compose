...
- `start-serena.sh` / `start-serena.ps1`: Read `LOCAL_PROJECT_PATH` from `.env` and start Serena with `docker compose up -d`
- `stop-serena.sh` / `stop-serena.ps1`: Stop and remove Serena with `docker compose down`

## Usage (agent or human)

1) Copy `.env.example` to `.env` and set `LOCAL_PROJECT_PATH` to the local project path Serena should activate.
...
The selected project path is mounted into the container at `/workspaces/active-project` and activated automatically on startup using `serena start-mcp-server --project /workspaces/active-project`.

## Agent Instructions

When the user asks to start Serena or to run a local Serena MCP server, ensure `.env` exists with `LOCAL_PROJECT_PATH` set to the desired project path. Do not prompt at runtime; the start scripts read from `.env` and Serena auto-activates that project at startup via `--project`. Run the start script from this folder, then share the MCP endpoint URL. Use the stop script to tear down the container when done.
